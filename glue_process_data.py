...
def process_data(data):
    ...
    for item in data:
        # Skip trips that never got an end event (status stuck at 'in_progress').
        # These have no real fare data yet, so including them would skew the
        # daily averages downward.
        if item.get('status') != 'completed':
            incomplete_trip_count += 1
            continue
    ...
    print(f"Skipped {incomplete_trip_count} incomplete (non-completed) trips.")
...
def main():
    ...
    # Process data to get aggregates (only counts trips with status == 'completed')
